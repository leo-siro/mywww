...
(function ($) {
  function TextEditor(args) {
    ...
    this.init = function () {
      ...
      // 要素を抜ける時にEdiotrを終了するように修正
      // $input.on("blur", function () {
      //   if (scope.validate().valid) {
      //     scope.applyValue(scope.args.item, scope.serializeValue());
      //   }
      //   args.grid.setActiveCell();
      // });
    };
      ...
      $input.datepicker({
        ...
        beforeShow: function () {
          ...
          if ($input.parents('.ui-dialog').length > 0) {
            setTimeout(function(){
              $('.ui-datepicker').css('z-index',String(parseInt($input.parents('.ui-dialog').css('z-index'),10) + 1));
            },0);
          }
        },
        onClose: function () {
